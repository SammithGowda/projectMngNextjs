  ...
  return (
    <div className="p-8">
      ...
      {searchResults&&searchResults.tasks && searchResults.tasks?.length > 0 && (
        ...
        {searchResults&&searchResults.tasks?.map((task) => (
              <TaskCard key={task.id} task={task} />
            ))}
          {searchResults&&searchResults.projects && searchResults.projects?.length > 0 && (
              <h2>Projects</h2>
            )}
            {searchResults&&searchResults.projects?.map((project) => (
